/* =====================================================
   ...
   - UPDATED: Reduced line spacing (V_SPACING)
   - UPDATED: Font List (Removed WordsTakenDemo, CAMPUS, White_On_Black; Added Spicy_Sale, Freakshow, Bouncy-Thin)
   - UPDATED: Defaults (Font: Spicy_Sale, Size: 80, Pattern: pattern6.gif)
   - FIXED: Reset Button Logic (Fully resets to initial state)
   ===================================================== */
...
const btnClearSel = document.getElementById('btnClearSel'); // This is the Icon Reset Button
const btnReset = document.getElementById('btnReset');       // This is the Bottom Dock Reset Button
...
// [UPDATED] 초기 기본값 설정: Spicy_Sale, Size 80, Pattern 6
const INITIAL_BASE = {
  font: 'Spicy_Sale',
  ...
  shadowY: 4,
  shadowBlur: 8,
  pattern: 'pattern6.gif', // Default Pattern 6
  strokePattern: null,
  effect: 'none'
};
...
/* UPDATED FONT LIST - 10 Fonts */
const FONT_LIST = [
  'Darling',
  'Bouncy-Thin-PERSONAL_USE_ONLY',
  'mieszkanie9',
  'Freakshow',
  'built_titling_el_it',
  'AngelicWar',
...
function layoutText(){
  ...
  // UPDATED: Line Spacing reduced (1.2 -> 1.0)
  const V_SPACING = 1.0; 
...
function drawAllText(ctx = mainCtx) {
  ...
  Object.entries(strokeBatches).forEach(([patKey, indices]) => {
      if (patKey === 'null') {
          ...
          // UPDATE PATH TO POP
          const imgKey = `./assets/pop/patterns/${patKey}`;
  ...
  Object.entries(fillBatches).forEach(([patKey, indices]) => {
    ...
    function updateStickerSlot(slotElement, slotIndex) {
      ...
      // UPDATE PATH TO POP
      thumb.style.backgroundImage = `url(./assets/pop/stickers/${currentFilename})`;
    }

    function addSticker(file){
      // UPDATE PATH TO POP
      const key = `./assets/pop/stickers/${file}`;
    ...
    function bindEvents(){
      ...
      if(btnClearSel) btnClearSel.addEventListener('click', ()=>{ playClick(); resetAll(); }); // Icon Reset Button
      if(btnReset) btnReset.addEventListener('click', ()=>{ playClick(); resetAll(); }); // Dock Reset Button
    ...
    function setPattern(file){
      ...
      if(v) {
          // UPDATE PATH TO POP
          loadImage(`./assets/pop/patterns/${v}`).then(() => render());
      }
    }
    ...
    function applyRandom(){
        ...
        // UPDATED: Range changed to 9 (0~8) to match HTML's 8 patterns, and extension set to .gif
        const pid = Math.floor(Math.random()*9);
        const p = pid===0?null:`pattern${pid}.gif`;
        ...
        // UPDATE PATH TO POP
        const key = base.pattern ? `./assets/pop/patterns/${base.pattern}` : null;
        if(key) loadImage(key).then(()=>{ layoutText(); render(); }); else { layoutText(); render(); }
    }

    function resetAll(){
        // [UPDATED] 리셋 시에도 위쪽의 초기 기본값과 동일하게 설정
        base = { ...INITIAL_BASE };
        
        shadowEnabled=true; 
        editTarget='fill'; 
        textString='Writing text'; 
        inputText.value=textString; 
        updateCount();
        ...
        if(bgFileInput) bgFileInput.disabled=true;
        if(bgFileInput) bgFileInput.value = ''; // Clear file input

        applyBaseToControls(); 
        
        // Reload pattern and render
        if(base.pattern) {
            loadImage(`./assets/pop/patterns/${base.pattern}`).then(() => {
                layoutText(); 
                render();
            });
        } else {
    ...
    (async function init(){
        ...
        // UPDATE PATH TO POP
        $$('#patternGrid .patBtn').forEach(btn=>{ let f = btn.dataset.pattern; if(f) btn.style.backgroundImage = `url(./assets/pop/patterns/${f})`; });
        ...
        // Preload default pattern if exists
        if(base.pattern) {
            loadImage(`./assets/pop/patterns/${base.pattern}`).then(() => {
