...
function createMetalTexture(w, h) {
  ...
  // MODIFIED for less top brightness, sharper highlight
  grad.addColorStop(0.00, '#2a2a2a');
  grad.addColorStop(0.20, '#555555');
  grad.addColorStop(0.40, '#888888');
  grad.addColorStop(0.47, '#bbbbbb');
  grad.addColorStop(0.49, '#ffffff');
  grad.addColorStop(0.495, '#ffffff');
  grad.addColorStop(0.50, '#000000');
  grad.addColorStop(0.55, '#333333');
...
function createBoldTexture(w, h) {
  ...
  grad.addColorStop(1.00, 'rgba(0, 0, 0, 0.95)'); 
...
function drawAllText(ctx = mainCtx) {
  ...
  Object.entries(strokeBatches).forEach(([patKey, indices]) => {
      if (patKey === 'null') {
          ...
          const imgKey = `./assets/glam/patterns/${patKey}`;
...
function drawEffectBatch(ctx, indices, effectType, bounds, cx, cy, vOffset) {
    ...
    if(effectType === 'metal') {
        ...
        // [수정됨]: overlay -> source-atop 으로 변경하여 대비 효과 강화
...
function loadImage(src){
  ...
  return new Promise(async (res) => { // Make executor async
    ...
    if (isGifExt && window.SuperGif) {
        try {
            // 1. Fetch file to check header (Magic Number)
            const response = await fetch(src);
            if (!response.ok) throw new Error("Fetch failed");
            
            const blob = await response.blob();
            const buffer = await blob.arrayBuffer();
            const headerArr = new Uint8Array(buffer).subarray(0, 3);
            const header = String.fromCharCode(...headerArr);

            // 2. Check if Real GIF
            if (header !== 'GIF') {
                // Fake GIF (PNG/JPG renamed), load as static
                console.log("Detected Fake GIF (Static Image):", src);
                loadStatic();
                return;
            }

            // 3. Real GIF - Load with SuperGif (No Timeout)
            const img = document.createElement('img');
            // Use Blob URL to save bandwidth (we already fetched it)
            ...
            console.warn("GIF Validation failed, falling back to static:", e);
            loadStatic();
        }
    } else {
        loadStatic();
...
function updateStickerSlot(slotElement, slotIndex) {
  ...
  thumb.style.backgroundImage = `url(./assets/glam/stickers/${currentFilename})`;
}

function addSticker(file){
  const key = `./assets/glam/stickers/${file}`;
...
function setPattern(file){
  ...
  if(v) {
      loadImage(`./assets/glam/patterns/${v}`).then(() => render());
  }
}
...
function applyRandom(){
    ...
    const key = base.pattern ? `./assets/glam/patterns/${base.pattern}` : null;
    if(key) loadImage(key).then(()=>{ layoutText(); render(); }); else { layoutText(); render(); }
}

function resetAll(){
    base = {font:'Darling', size:120, rot:0, fill:'#ff1493', stroke:6, strokeColor:'#000000', shadowX:4, shadowY:4, shadowBlur:8, pattern:null, strokePattern:null, effect:'none'};
    ...
    bgMode='transparent'; bgImageObj=null; bgColor='#000000';
...
(async function init(){
    ...
    $$('#patternGrid .patBtn').forEach(btn=>{ let f = btn.dataset.pattern; if(f) btn.style.backgroundImage = `url(./assets/glam/patterns/${f})`; });
    inputText.value = textString; updateCount(); applyBaseToControls(); 
    
    // FIX: Force re-render when the default font is actually loaded
    bindEvents();
    layoutText(); 
    render();
    
    const fontStr = `${base.size}px '${base.font}'`;
